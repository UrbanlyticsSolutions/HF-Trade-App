...
def run_single_config(params, period='2025'):
    ...
    tc = TradeConfig(
        ...
        regime_vol_percentile=0.40,
        regime_trend_percentile=0.30,
        regime_skip_first_bar=True,
        regime_rsi_buffer=params.get('regime_rsi_buffer', 10),
        regime_size_reduction=params.get('regime_size_reduction', 0.10),
...
def create_objective(period='2025'):
    ...
    def objective(trial):
        ...
        ret_score = m['ret'] / 600  # baseline ~555%, so 600 normalizes
        sharpe_score = m['sharpe'] / 8
        pf_score = min(m['pf'] / 4, 1)
        wr_score = (m['wr'] - 55) / 25  # penalize below 55%
        trade_score = min(m['trades'] / 350, 1.0)  # reward more trades (baseline ~350)
        dd_penalty = max(0, (m['max_dd'] - 10) / 10)  # only penalize DD > 10%
...
def run_optimization(n_trials=300, period='2025'):
    """Run optimization with Optuna TPE sampler (single process, in-memory)."""
    cap = cfg.initial_capital()

    print('=' * 70)
    print(f'  COMPLETE STRATEGY OPTIMIZATION')
    print(f'  Optuna TPE | {n_trials} trials | single-process in-memory')
